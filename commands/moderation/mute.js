...
module.exports = async (client, msg, args) => {
    ...
    if (ExistRole[0].roleInit, ExistRole[0].roleMute !== 'none') {
        ...
        if (user) {
            if (msg.guild.member(user).roles.cache.has(ExistRole[0].roleMute)) return msg.channel.send('O usuario ja esta mutado!');

            try {
                await msg.guild.member(user.id).roles.add(ExistRole[0].roleMute);
                return msg.channel.send(`O usuario ${user} foi mutado`);
            } catch (e) {
                console.log(e);
            }
