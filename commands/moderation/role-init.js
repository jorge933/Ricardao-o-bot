...
module.exports = async (client, msg, argumentos) => {
    ...
    if (ExistRole[0].roleInit === 'none') {
        const ExistRole = msg.guild.roles.cache.get(roleId);

        if (ExistRole) {
            await db.run('UPDATE guildManager SET roleInit = ? WHERE guildId = ?', [roleId, msg.guild.id]);
            await db.close()
            return msg.channel.send('Cargo padrão adicionado!');
        ...
        if (ExistRole) {
            await db.run('UPDATE guildManager SET roleInit = ? WHERE guildId = ?', [roleId, msg.guild.id]);
            await db.close()
            return msg.channel.send('Cargo padrão atualizado!');
        } else return msg.channel.send('Este cargo não existe')
    }
}